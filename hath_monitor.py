...
I18N = {
    ...
    "de": {
        ...
        "trust": "Vertrauen", "quality": "Qualität", "hitrate": "Trefferquote", "hathrate": "Hath-Rate",
    ...
    "ja": {
        ...
        "version_info": "状態: {type}\nバージョン: {old} -> {new}", "changelog_label": "変更履歴",
    ...
    "ar": {
        ...
        "quota_title": "تذكير بحصة H@H اليومية", "quota_body": "حصة الأرشيف المجانية الحالية:\n{quota}",
        "status_change": "تغيير الحالة", "status_report": "تقرير الحالة", "client_msg_title": "H@H {type}: {name}",
        "online": "متصل", "offline": "غير متصل", "cur_status": "الحالة الحالية", "detail": "التفاصيل",
        "region": "المنطقة", "address": "العنوان", "version": "الإصدار", "speed": "عرض النطاق الترددي",
...
def get_t(lang, key, **kw):
    t = I18N.get(lang, I18N["zh"]).get(key, key)
        ...
        with sqlite3.connect(db) as conn:
            for s, f, l in [("official", check_official, "official"), ("rust", check_rust, "rust")]:
                ...
                if force_push or not last or last[0] != v:
                    ...
                    body = get_t(lang, "version_info", type=get_t(lang, "update_type_update" if last else "update_type_new"), old=last[0] if last else "?", new=v)
                    if len(rest) > 1 and rest[1]: body += f"\n\n{get_t(lang, 'changelog_label')}:\n{rest[1]}"
                    send_notify(ncfg, get_t(lang, "new_version_found", label=get_t(lang, l), ver=v), body)
                ...
                if d:
                    if "version" in d:
                        ...
                        if force_push or not last or last[0] != v:
                            ...
                            send_notify(ncfg, get_t(lang, "new_version_found", label=get_t(lang, "web"), ver=v), get_t(lang, "version_info", type=get_t(lang, "update_type_update"), old=last[0] if last else "?", new=v))
                    if mcfg.get("check_quota") and "quota" in d:
                        ...
                        if force_push or not lq or datetime.fromisoformat(lq[0]).date() < datetime.now(timezone.utc).date():
                            ...
                            send_notify(ncfg, get_t(lang, "quota_title"), get_t(lang, "quota_body", quota=q))
                    if mcfg.get("check_clients") and "clients" in d:
                        for c in d["clients"]:
                            ...
                            if force_push or not ls or ls[0] != c['status_key']:
                                ...
                                st_n = get_t(lang, c['status_key'])
                                body = f"{get_t(lang, 'cur_status')}: {st_n}\n{get_t(lang, 'detail')}: {get_t(lang, ls[0]) if ls else '?'} ➔ {st_n}\n{get_t(lang, 'region')}: {c['region']}\n{get_t(lang, 'address')}: {c['ip']}:{c['port']}\n{get_t(lang, 'version')}: {c['version']}\n{get_t(lang, 'speed')}: {c['max_speed']}\n{get_t(lang, 'trust')}: {c['trust']}\n{get_t(lang, 'quality')}: {c['quality']}\n{get_t(lang, 'hitrate')}: {c['hitrate']}\n{get_t(lang, 'hathrate')}: {c['hathrate']}"
                                send_notify(ncfg, get_t(lang, "client_msg_title", type=get_t(lang, "status_change" if ls and ls[0] != c['status_key'] else "status_report"), name=c['name']), body)
